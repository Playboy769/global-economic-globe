...
- **TickerInsight v2.6（2026-09-14）**：面板本體整體下移一列——四個區塊改佔 **rows 8–24**（`TI_BODY = 1` 加在所有
  `RR4_TOP + n` 的本體列上；`TI_HHDR` 9、`TI_H1` 10、`TI_HLAST` 24 不變所以交易紀錄少一列、`TI_BOTTOM` 25 放「+n older」），
  **目標價輸入格 `TI_TARGET_CELL` 改 L22**（原 L21，UP 讀不到 L22 時退回讀 L21 一次）。row 7 留空當標題與本體的間隔，
  `ClearTickerData` 仍從 row 7 清起。
...
- **WATCHLIST（v4.9.1；⚠️ 2026-09-25 起 RR4 頁這塊只是 Watch 頁的唯讀摘要，資料在 tblWatch，見下方「Watch 頁」；以下是舊 entry-row 時代的描述）**：圖表帶左側 B25:E38——標題 25、表頭 26、**row 27 是輸入列**
  ...
  **2026-09-26：原輸入列（第 28 列）退役後一直空著，使用者要求放內容——現在清單是 rows 28–35（8 格，`RR4_WL_FIRST = 28`，`RR4_WL_ENTRY` 與 `WatchlistPaintEntryRow` 已刪），摘要取 Watch 頁排序的前 8 檔。**
...
- **RRG（RG）頁（2026-09-12）**：`RRG.bas`，`sector-rotation-system/rrg_dynamic.py` 的 Excel 版，
  ...
  **同頁下半：美股資金流向 2D（`money_flow_chart.py` / `money_flow_ranking.py`）**——同一次抓取改拿整組
...
- **Thesis Library v2 — 主題筆記工作台（2026-09-13 晚，取代下面的 v1）**：`modThesis.bas` 整支重寫，照使用者給的
  ...
  **平常只畫最新一次 CALL DATE 的筆記；QUERY 有打代號時，該代號的全部筆記依 CALL DATE 新→舊分組畫出**（2026-09-13 晚加），代號粗體＋下方灰色最新日期，較舊的每次法說日期寫在該組第一列。雙擊表頭第一格＝區塊排序（最新法說日↔代號 A–Z，`THSORT`）；
...
- **2026-09-25 兩項調整（使用者）**：① **K 棒改「紅漲白跌」、黑色 0.75pt 邊框**（漲＝RGB(220,60,60) 實心、跌＝純白實心，
  影線白色（**2026-09-25 稍晚：高低連線粗細 0.75→0.25pt**，兩張 K 線圖；⚠️ 在 96dpi 的匯出圖裡 0.25pt 與 0.75pt 都會被畫成 1 像素、逐像素比對完全相同，只有高 DPI 螢幕才看得出差別——要再細只能調暗影線顏色或改自建誤差線），量柱同紅／白＋黑框；圖表背景維持黑底所以黑框在黑底上幾乎看不見，靠填色區分）。邊框會吃掉細棒，所以漲跌柱
...
- **連漲／連跌根數（2026-09-26，使用者要求）**：單檔 K 線圖（`BIAS_K_R1`／`BIAS_K_R4` 兩張）在連續同向的 K 棒上標**累計根數**——連漲畫在該棒**最高價上方**、連跌畫在**最低價下方**，**只標 ≥3 根**（`STREAK_MIN` 固定常數、沒有輸入格；第 1、2 根照算但不標，所以第一個看到的數字是 3）。白色粗體 7pt，做法是兩條只有資料標籤的次座標軸折線（`STREAK UP`／`STREAK DN`，無線無標記，同一組對數軸範圍，圖例項目已刪；隱藏資料塊多 4 欄、`KB_LAST` 25→29，`AddStreakLabels`）。**MODE <GO> 輸入格**（TICKER／GROUP 右邊，實際 G 欄，下拉 `CLOSE,RED`、預設 CLOSE、`B!` 重建會保留）決定「上漲」定義：CLOSE＝收盤>前一日收盤、RED＝收盤>開盤（紅 K）；平盤（相等）**兩個方向都中斷**。改 MODE 只重畫目前單檔圖（`BiasChange` 的 MODE 分支→`RefreshBiasQuery`），不動熱力表／群組表。計數在**完整 OHLC 歷史**上算完再切成圖上那段（`RefreshBiasQuery` 現在整段抓 `FetchBiasOhlc` 再切片），所以圖最左邊那根的連續數是對的。實測 AAPL（CLOSE 最長 7、RED 8，圖上標籤逐點與本地重算 0 差異）、2330（.TW 回退）、MODE 切 RED 後標籤數 70→79、左緣第 0 根 RED 模式顯示連跌 4（只算圖上那段會是 0）。

- **COMPARE <GO>／RANGE <GO>（2026-09-26，使用者「比較個股績效線圖，log2 座標」）**：Bias 頁輸入列多兩格——**COMPARE**（合併 H:N，逗號／全形逗號／空白分隔代號，認 `@PORT`＝RR4 持倉、`@WATCH`＝Watch 名單，上限 `CMP_MAX`=12）與 **RANGE**（下拉 3M/6M/1Y/2Y/3Y/YTD，預設 1Y）。改任一格就 `RunCompare`（`BiasChange` 分派；merged 格 `Target.CountLarge` 是 7，所以事件門檻改 `> CMP_SPAN`）、`B!` 重建時若 COMPARE 有值會一併重畫。**一張大圖多條線**（`BIAS_CMP`，錨在兩組 R／K 線圖正下方、寬同兩組合計 1532、高 520）：還原收盤（adjclose）各自原幣報酬，**起點歸一為 100**，**價格軸 log2**、界限錨在 100（…50/100/200/400…，一格＝翻倍或腰斬）、虛線標 100 基準；**依各檔最終報酬由高到低排序**，圖例寫成「代號 +xx.x%」，配色依輸入順序固定 12 色。日期＝各檔交易日聯集，休市的市場沿用前一收盤；區間內才上市的標的從自己第一根起算 100（起點與其他線不同）；起點錨＝各檔「區間起日當天或之前最後一根」中最晚的那個日期（YTD＝去年最後交易日）。資料塊放隱藏 ZA 側、`CMP_DATA_OFF`=31 欄後（K 線資料塊之後），`ClearCompare` 清 1000 列。價格走 `modvolatility.GetHistoricalData`（10y adjclose、裸台股碼自動試 .TW/.TWO），每檔約 1–2 秒。⚠️ 抓圖驗證：隱藏實例 `Chart.Export` 只寫 0 位元組，改 `xl.Visible=$true` 後 `Chart.ChartArea.Copy()` 再用 `System.Windows.Forms.Clipboard.GetImage()` 存 PNG 就看得到（2026-09-26 實測有效）。實測 12 檔 1Y／5 檔 YTD／@PORT 9 檔皆正確出圖。
...
- **歷史漏記的股利**：由使用者提供清單（日期／代號／實收），再用 COM 一次寫進 Transactions 表尾——**FIFO 照列序、不照日期**，但股利不進 FIFO，所以位置不影響結果。
  **2026-09-26 已補**（備份 `backup-pre-divimport-20260926.xlsm`）：3017.TW 第 95 列 Net_Amount 19→**177**（使用者確認實收）；新增第 96–99 列美股股利（Action=Dividend、Broker 國泰世華）——AAPL 8/10 US$0.36、AMAT 8/20 US$0.37、MSFT 8/20 US$1.56、GOOGL 9/4 US$1.02，**皆為預扣 30% 後實收（每股股利×除息日持股×0.7 四捨五入到分）**，日期是**除息日**而非入帳日（Yahoo 沒有入帳日，只影響 HistoryLog 累計線歸屬日）。這次補登當下 VBE 在中斷模式、`Application.Run` 不能用，所以 Realized 頁要等下次 `UP` 才會出現 5 列 `DIVIDEND`。
